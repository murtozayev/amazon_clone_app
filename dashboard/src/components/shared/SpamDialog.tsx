...
export default function SpamDialog({ children, data }: PROPS) {

    const useClient = useQueryClient()

    const { mutateAsync: spamAsync, isPending: isSpamming } = useMutation({
        async mutationFn({ id, time }: { id: string, time: Date }) {
            const res = await api.post(`/dashboard/users/setspam/${id}`, { status: "spam", time }, { headers: { "Content-Type": "application/json" } })
    ...
    async function onSpam(e: FormEvent<HTMLFormElement>, id: string) {
        e.preventDefault()
    ...
    return (
        <Dialog>
            ...
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle>Spam deb belgilash</DialogTitle>
                    <DialogDescription>
                        Foydalanuvchi qandaydir muammo qilgan bo'lsa uni imkoniyatlarini cheklash
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={(e) => onSpam(e, data._id)} className="flex items-center gap-2">
                    <div className="grid flex-1 gap-2">
                        ...
                        <Input
                            ...
                            min={0}
                            max={20}
                            placeholder="Kunlarda belgilang"
                            name="time"
                        />
                    </div>
                    <Button type="submit">
                        {isSpamming ? <Loader className="animate-spin" /> : "Spam deb belgilash"}
                    </Button>
                </form>
